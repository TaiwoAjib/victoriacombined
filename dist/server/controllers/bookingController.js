...
exports.createPaymentIntent = exports.checkInBooking = exports.updateBooking = exports.createAdminBooking = exports.createBooking = exports.getBookings = void 0;
...
const getStylistSurcharge = (stylist, styleId) => {
    if (!stylist)
        return 0;
    // Check for style-specific surcharge first
    if (stylist.styleSurcharges && styleId) {
        const styleSurcharges = stylist.styleSurcharges;
...
const isStylistWorking = (stylist, bookingDate, timeStr) => {
    if (!stylist || !stylist.workingHours)
        return true;
    ...
    try {
        ...
        const bookingsWithPrice = await Promise.all(bookings.map(async (booking) => {
            ...
            if (booking.styleId && booking.categoryId) {
                const pricing = await prisma_1.default.stylePricing.findUnique({
                    where: {
                        styleId_categoryId: {
            ...
            const { payments: _payments, ...bookingRest } = booking;
            return {
                ...bookingRest,
...
const createBooking = async (req, res) => {
    try {
        ...
        // Validate Style
        if (styleId) {
            const style = await prisma_1.default.style.findUnique({ where: { id: styleId } });
            if (!style) {
            ...
            if (stylistId) {
                ...
                const onLeave = await prisma_1.default.stylistLeave.findFirst({
                    where: {
                        stylistId,
                ...
                const capableStylist = await prisma_1.default.stylist.findFirst({
                    where: {
                        id: stylistId,
                ...
                if (!capableStylist) {
                    const stylistExists = await prisma_1.default.stylist.findUnique({ where: { id: stylistId } });
                    if (stylistExists) {
        ...
        if (!userId) {
            ...
            let user = await prisma_1.default.user.findUnique({ where: { email: guestDetails.email } });
            if (!user) {
                ...
                if (user.notificationConsent) {
                    const { subject, html } = await emailService_1.emailService.getGuestCredentialsContent(guestDetails.fullName, randomPassword);
                    await notificationQueueService_1.notificationQueue.add('EMAIL', 'GN', guestDetails.email, html, subject, { type: 'GUEST_CREDENTIALS', userId: user.id });
                }
            }
            else {
            ...
            if (guestDetails) {
                ...
                if (typeof guestDetails.smsConsent === 'boolean') {
                    data.notificationConsent = guestDetails.smsConsent;
                }
        ...
        if (user && user.notificationConsent && user.email) {
            const { subject, html } = await emailService_1.emailService.getBookingConfirmationContent(user.fullName, serviceDisplayName, date, time, !!guestDetails);
            await notificationQueueService_1.notificationQueue.add('EMAIL', 'BN', user.email, html, subject, { bookingId: result.id, userId: user.id });
        }
...
const createAdminBooking = async (req, res) => {
    try {
        ...
        let customer = null;
        if (customerId) {
            customer = await prisma_1.default.user.findUnique({ where: { id: customerId } });
        }
        else if (guestDetails) {
            customer = await prisma_1.default.user.findUnique({ where: { email: guestDetails.email } });
            if (!customer) {
                const randomPassword = Math.random().toString(36).slice(-8);
                const passwordHash = await bcryptjs_1.default.hash(randomPassword, 10);
                customer = await prisma_1.default.user.create({
                    data: {
                        fullName: guestDetails.fullName,
                        email: guestDetails.email,
                        phone: guestDetails.phone,
        ...
        if (stylistId) {
            ...
            if (styleId) {
                const capable = await prisma_1.default.stylist.findFirst({
        ...
        if (customer.notificationConsent && customer.email) {
            ...
            try {
                const { subject, html } = await emailService_1.emailService.getBookingConfirmationContent(customer.fullName, serviceDisplayName, date, time, false);
                await notificationQueueService_1.notificationQueue.add('EMAIL', 'BN', customer.email, html, subject, { bookingId: booking.id, userId: customer.id });
            }
            catch (e) {
...
const updateBooking = async (req, res) => {
    try {
        ...
        if (stylistId !== undefined) {
            if (stylistId === '--select a stylist--' || stylistId === '' || stylistId === null) {
                ...
                if (effectiveStyleId) {
                    ...
                    stylistObj = await prisma_1.default.stylist.findUnique({ where: { id: stylistId } });
                }
        ...
        if (status === 'completed' && booking.status !== 'completed') {
            ...
            const category = updatedBooking.category;
            if (customer && customer.notificationConsent && customer.email) {
                const { subject, html } = await emailService_1.emailService.getBookingCompletionContent(customer.fullName, category?.name || 'Service');
                await notificationQueueService_1.notificationQueue.add('EMAIL', 'BN', customer.email, html, subject, { bookingId: id, userId: customer.id });
            }
        }
