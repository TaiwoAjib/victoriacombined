...
const notificationQueueService_1 = require("./notificationQueueService");
exports.reminderService = {
        ...
        try {
            ...
            for (const booking of bookings) {
                ...
                if (bookingHour === currentHour) {
                    await exports.reminderService.sendReminderForBooking(booking);
                }
            }
        }
        catch (error) {
     ...
     */
    sendReminderForBooking: async (booking) => {
        try {
            const { customer, style, category, bookingDate, bookingTime } = booking;
            if (!customer)
                return;
            if (customer.notificationConsent === false)
                return;
            ...
            const alreadySent = await prisma_1.default.notification.findFirst({
                where: {
                    metadata: {
                        path: ['bookingId'],
                        equals: booking.id
            ...
            if (customer.email) {
                ...
                const { subject, html } = await emailService_1.emailService.getBookingReminderContent(customer.fullName, serviceName || 'Service', dateStr, timeStr);
                await notificationQueueService_1.notificationQueue.add('EMAIL', 'BN', customer.email, html, subject, { bookingId: booking.id, type: 'REMINDER' });
            }
        }
        catch (error) {
