...
const client = (accountSid && accountSid.startsWith('AC') && authToken) ? (0, twilio_1.default)(accountSid, authToken) : null;
...
exports.smsService = {
     ...
     */
    getBookingConfirmationContent: async (name, date, time) => {
        try {
     ...
     */
    getBookingReminderContent: async (name, date, time) => {
        try {
            const template = await prisma_1.default.notificationTemplate.findUnique({
     ...
     */
    getBookingCompletionContent: async (name) => {
        try {
            const template = await prisma_1.default.notificationTemplate.findUnique({
     ...
     */
    getPaymentReceiptContent: async (name, amount) => {
