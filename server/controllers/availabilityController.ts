...
export const getAvailability = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let requestedDuration = duration ? parseInt(duration as string) : 0;

    if (!requestedDuration && styleId && categoryId) {

        const stylePricing = await prisma.stylePricing.findFirst({
            where: {
    ...
    const bookings = await prisma.booking.findMany({
        where: {
            ...
            ...(excludeBookingId ? { id: { not: excludeBookingId as string } } : {})
        },
        select: {
            id: true,
            bookingDate: true,
    ...
    while (loopDate <= end) {
        ...
        for (let slotStart = openMinutes; slotStart + requestedDuration <= closeMinutes; slotStart += 60) {
            ...
            for (const s of activeStylists) {
                ...
                if (s.workingHours) {
                    const schedule = (s.workingHours as any)[dayName];
                    if (schedule) {
                        ...
                        if (slotStart < sStart || slotEnd > sEnd) continue;
                    }
                }
